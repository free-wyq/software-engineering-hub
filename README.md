...
> **安装**：复制下方 [安装：给任意智能体的提示语](#安装给任意智能体的提示语) 发给任何智能体,它会自动克隆仓库并把根目录的 `se-hub/` 装进自己的技能目录(如 `.claude/skills/se-hub`、`.codebuddy/skills/se-hub`)并启用,无需其他人工步骤。
>
> **通用性说明**：本仓库是**技能开发库**（通用类型）——在这里开发、维护软件工程方向的技能,不绑定任何具体业务项目。仓库即技能的唯一源（master）：`se-hub/` 以标准 skill 目录结构（`SKILL.md` + `references/` + 可选 `scripts/`）置于仓库根目录,可被任何采用 Claude Code skill 机制的智能体/项目直接引用或复制使用；新技能开发也按同一结构在本仓库内进行,随 git 版本化。

## 安装：给任意智能体的提示语

把下面这段话原样发给任何支持 skill 机制的智能体（Claude Code / CodeBuddy / Cursor / Trae 等）,它即可自行完成安装。已按「不绑定特定平台」编写——技能目录路径由智能体根据自身环境判断：

```text
请帮我安装一个软件工程技能库。

仓库地址：https://github.com/free-wyq/software-engineering-hub.git

这是什么：
- 采用 Claude Code skill 机制编写的分层技能库。仓库根目录下的 se-hub/ 目录即技能本体（SKILL.md + references/ 子文档）,结构为「一个 hub 入口 + references 渐进披露」。
- 这是通用方法论技能库,不绑定任何具体业务项目。安装后只读取、不修改其中内容。

请执行：
1. 克隆（或下载）该仓库到任意临时目录；
2. 将仓库根目录下的 se-hub/ 整个目录,复制到你当前环境的技能目录下,目录名保持 se-hub。例如 .claude/skills/se-hub、~/.claude/skills/se-hub、.codebuddy/skills/se-hub,或你所在平台约定的等效位置——具体由你判断；若确实无法确定,先问我再动手；
3. 校验安装结果：se-hub/SKILL.md 存在,且 se-hub/references/ 下 requirement、design、coding、integration、testing、delivery 六个子目录都完整；
4. 清理临时克隆产物,然后告诉我：你把技能装到了哪个绝对路径、该技能是否已被你的环境识别启用。

如果无法访问该仓库（内网限制、无权限等）,直接说明原因即可,不要编造内容或凭记忆生成技能文件。
```

> 智能体不具备联网/克隆能力时,退化为手动方式：自行下载仓库,把 `se-hub/` 拷到对应技能目录即可,效果相同。
